...
<div class="bg-white p-6 rounded-lg shadow-md">
    ...
    <form action="{{ route('usersAdmin.update', Crypt::encryptString($user->id_user)) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <textarea id="alamat" name="alamat" class="w-full p-2 border border-gray-300 rounded-lg">{{ old('alamat', $user->alamat) }}</textarea>
            @error('alamat')
                <div class="text-red-500 bg-red-100 border border-red-400 rounded-md p-2 mt-2">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            @if ($user->foto_user)
                <div class="mt-2">
                    <img src="{{ asset('storage/' . $user->foto_user) }}" alt="Foto User" class="w-32 h-32 object-cover rounded-lg">
                </div>
            @endif
            @error('foto_user')
                <div class="text-red-500 bg-red-100 border border-red-400 rounded-md p-2 mt-2">{{ $message }}</div>
        ...
        <div class="flex space-x-4">
            ...
            <a href="{{ route('usersAdmin.index') }}" class="bg-gray-300 text-gray-700 px-4 py-2 rounded-lg">Batal</a>
        </div>
    </form>
</div>
@endsection
